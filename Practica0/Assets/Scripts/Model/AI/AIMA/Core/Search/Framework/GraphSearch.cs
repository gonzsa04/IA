...
{ 
    ...
    public class GraphSearch : QueueSearch
    {
        ...
        private Dictionary<System.Object, Node> frontierState = new Dictionary<System.Object, Node>();
        ...
        public override List<Operator> search(Problem problem, Queue<Node> frontier)
        {
            // initialize the explored set to be empty
            explored.Clear();
            frontierState.Clear();
                ...
                Problem problem)
        {
            ...
            foreach (Node cfn in expandNode(nodeToExpand, problem)) {
                ...
                Node frontierNode;
                frontierState.TryGetValue(cfn.getState(), out frontierNode); // El funcionamiento de este Diccionario parece estar fallando (antes es que tenía notación de array y seguramente no era válido)
