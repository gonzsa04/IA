namespace UCM.IAV.Puzzles {
    ...
    public class BlockBoard : MonoBehaviour {
        ...
        private void GenerateCasillas(SlidingPuzzle puzzle)
        {
            if (puzzle == null) throw new ArgumentNullException(nameof(puzzle));
            ...
            for (var r = 0u; r < rows; r++)
            {
                for (var c = 0u; c < columns; c++)
                {
                    ...
                    if (casilla == null)
                    {
                        casilla = Instantiate(casillaPrefab,
                            new Vector3(-((casillas.GetLength(1) / 2.0f) * POSITION_FACTOR_C - (POSITION_FACTOR_C / 2.0f)) + c * POSITION_FACTOR_C, 0,
        ...
        public void ResetBlockBoard(SlidingPuzzle puzzle)
        {
            var rows = casillas.GetLength(0);
            var columns = casillas.GetLength(1);
